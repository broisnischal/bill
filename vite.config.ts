...
export default defineConfig(({ mode }) => ({
  ...
  // nepali-date-converter ships CJS as `main` and ESM only as `module`, so dev SSR
  // externalizes it and hands back a double-wrapped default that cannot be constructed.
  // Letting Vite transform it keeps dev and the production build on the same interop.
  ssr: {
    noExternal: ["nepali-date-converter"],
  },
